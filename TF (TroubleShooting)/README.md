...
tsc/
├── README.md
├── tsc-scenario1-oomkilled-hpa.sh      # OOMKilled 반복 + HPA 미작동 복합 장애
├── tsc-scenario2-pvc-storageclass.sh   # PVC Pending + StorageClass 불일치
└── tsc-scenario3-rbac-sa.sh            # RBAC 권한 누락 / ServiceAccount 오설정
```
---
## 시나리오 상세
...
```bash
# 전체 네임스페이스, 재시작 3회 이상
./tsc-scenario1-oomkilled-hpa.sh

# 특정 네임스페이스, 임계 5회
./tsc-scenario1-oomkilled-hpa.sh --namespace production --threshold 5
```
...
PVC `Pending` 상태는 기본 탐지 가능하지만, **왜** Pending인지는 Events 로그를 파싱해야만 알 수 있습니다. 원인에 따라 조치 방법이 완전히 다릅니다.

**탐지 원인 분류:**

| 원인 코드 | 의미 | 대표 증상 |
|-----------|------|-----------|
| `NO_PROVISIONER` | StorageClass에 명시된 provisioner pod/CSIDriver 미존재 | SC 생성은 됐지만 실제 드라이버 미설치 |
| `WAIT_FIRST_CONSUMER` | 바인딩 모드가 `WaitForFirstConsumer`인데 파드가 스케줄 안 됨 | PVC만 있고 파드가 Pending |
| `TOPOLOGY_MISMATCH` | SC `allowedTopologies` zone과 실제 노드 zone 불일치 | 멀티존 클러스터에서 자주 발생 |
| `RESOURCE_QUOTA` | ResourceQuota 초과로 PVC 생성 차단 | 네임스페이스 스토리지 quota 부족 |
...
./tsc-scenario2-pvc-storageclass.sh

# 특정 네임스페이스, 2분 이상 Pending
./tsc-scenario2-pvc-storageclass.sh --namespace staging --age-minutes 2
```
...
### Scenario 3 — RBAC 권한 누락 / ServiceAccount 오설정 런타임 장애

**왜 딥한가?**

이미지 Pull도 성공하고, 파드도 `Running`인데 **앱이 k8s API를 호출하는 순간 403/401**이 발생합니다. 이 장애는 파드 이벤트에는 나타나지 않으며, 앱 로그를 직접 확인하거나 RBAC 설정을 분석해야 발견됩니다.

Operator 패턴, Argo Workflows, Flux, 커스텀 컨트롤러를 사용하는 환경에서 특히 자주 발생합니다.

**탐지 항목:**

| 항목 | 설명 |
|------|------|
| `automountServiceAccountToken: false` | SA 토큰 미마운트 + Running 파드 교차 확인 |
| SA에 RoleBinding/ClusterRoleBinding 없음 | 바인딩 누락으로 권한 0인 SA 탐지 |
| `ClusterRole` → namespace-scoped `RoleBinding` | cluster-scoped 리소스 접근 제한 가능성 경고 |
| `kubectl auth can-i` 주요 verb 검증 | get/list/watch on pods/services/configmaps |
| 파드 로그 RBAC 에러 패턴 탐지 | `403`, `Forbidden`, `RBAC` 키워드 (옵션) |
| `default` SA 사용 파드 경고 | 전용 SA 미설정 상태 권고 |

**실행 예시:**

```bash
# 기본 실행 (로그 검사 제외)
./tsc-scenario3-rbac-sa.sh

# 특정 네임스페이스 + 파드 로그까지 검사
./tsc-scenario3-rbac-sa.sh --namespace production --check-logs

# default SA 경고 제외
./tsc-scenario3-rbac-sa.sh --namespace production --skip-default-sa
```
...
| 권한 | `get`, `list` on pods, pvc, sa, rolebinding, hpa 등 |
| python3 | JSON 파싱 용도 (대부분 환경 기본 설치) |
| OS | Linux / macOS (bash 4.0+) |

> Windows 환경은 WSL2 또는 Git Bash에서 실행하세요.
...
```bash
chmod +x tsc-scenario1-oomkilled-hpa.sh
chmod +x tsc-scenario2-pvc-storageclass.sh
chmod +x tsc-scenario3-rbac-sa.sh
```
